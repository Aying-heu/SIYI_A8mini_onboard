...
void VisionModule::process_loop() {
    ...
    while (running_) {
        ...
        for (int idx1 : indexes1) {
            ...
            Z_ONNX(roi, mark_model_, boxes2, indexes2, conf_cls_names2);
            for(int idx2 : indexes2){
                    // 安全检查：确保 conf_cls_names2 有值，并且 worldPoints 中存在对应键且点集足够
                    std::string key;
                    if (idx2 >= 0 && idx2 < (int)conf_cls_names2.size()) key = conf_cls_names2[idx2];
                    auto it_wp = worldPoints_.find(key);
                    if (key.empty() || it_wp == worldPoints_.end() || it_wp->second.size() < 2) {
                        std::cerr << "[WARN] missing worldPoints for key='" << key << "' image_ts=" << row.nano_timestamp << std::endl;
                        data_manager_->pushDetectedImage(row);
                        continue;
                    }
                    object_points1.push_back(it_wp->second[0]);
                    object_points2.push_back(it_wp->second[1]);
       
                    // 求取目标中心点（相对于整图）
                    float cx = boxes2[idx2].x + boxes2[idx2].width / 2.0f + boxes1[idx1].x;
                    float cy = boxes2[idx2].y + boxes2[idx2].height / 2.0f + boxes1[idx1].y;
                    // 由于图像放大倍数，需要回归
                    float out_Cx = (cx - img_width / 2.0f) / actual_zoom + img_width / 2.0f;
                    float out_Cy = (cy - img_height / 2.0f) / actual_zoom + img_height / 2.0f;

                    // out_Cx = (out_Cx - img_width / 2.0f) *2 + img_width * 2.0f;
                    // out_Cy = (out_Cy - img_height / 2.0f) *2 + img_height * 2.0f;
                    out_Cx = (out_Cx - img_width/2.0f) * 2.0f + 2560/2.0f;
                    out_Cy = (out_Cy - img_height/2.0f) * 2.0f + 1440/2.0f;
                    image_points.push_back(cv::Point2f(out_Cx, out_Cy));
            }
        }
        object_points=object_points1;
        if (object_points.empty() || object_points.size()<4) {
            std::cout<<"couldn't find ABCD"<<std::endl;
            data_manager_->pushDetectedImage(row);
            continue;
        }
...
//     trt_options.trt_max_workspace_size = 4294967296; // 4GB
//     trt_options.trt_fp16_enable = 1;               // 开启 FP16 加速
