...
DataManager::DataManager(rclcpp::Node::SharedPtr node,const std::string& root_path_)
    ...
    data_config_.target_file_=data_config_.base_dir_+"/target_states.csv";
...
void DataManager::pushDetectedTargetMsg(prometheus_msgs::msg::Target target_msg) {
    std::lock_guard<std::mutex> lock(detected_target_msg__mtx_);
    // 这里使用 std::move，会将传入的临时副本内容“移动”到 vector 中
    detected_target_msg_.emplace_back(std::move(target_msg));
...
void DataManager::popUsedData(std::vector<UAV_Row>& uav_list, std::vector<Gimbal_Row>& gimbal_list,std::vector<Img_Row>& img_list,std::vector<prometheus_msgs::msg::Target>& target_list) {
    {
    ...
    }
    {
        ...
        if(detected_target_msg_.size()>1){
            ...
            target_list.swap(detected_target_msg_);
