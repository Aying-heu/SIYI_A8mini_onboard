...
void DataLogger::initialize_files() {
    ...
    // 3. 目标信息文件
    target_file_.open(config->target_file_, std::ios::app);
...
void DataLogger::run() {
    while (rclcpp::ok() && running_.load()) {
        ...
        std::vector<prometheus_msgs::msg::Target> target_list;

        // 1. 从数据中心取出所有“已用”数据（内部执行 swap，极快）
        data_manager_->popUsedData(uav_list, gimbal_list, img_list, target_list);
        ...
        for (auto& t : target_list) {
            ...
            target_file_ << target_time << "," << target_dt << ","
                         << t.px << "," << t.py << "," << t.pz << ","
                         << t.attitude[0] << "," << t.attitude[0] << "," << t.attitude[2]<<","
        ...
        //     idx+=1;
        //     if (!img_row.img.empty()) {
        //         // 保存图片到磁盘
        //         if(img_row.is_detected){
        //             std::string filename = "/detected/" + std::to_string(img_row.nano_timestamp) + ".jpg";
        //             std::string detected_path = data_manager_->get_config()->image_dir_ + filename;
        //             cv::imwrite(detected_path, img_row.img);
        //         }
        //         if(idx%5!=0) continue;
        //         std::string filename = "/raw/" + std::to_string(img_row.nano_timestamp) + ".jpg";
        //         std::string else_path = data_manager_->get_config()->image_dir_ + filename;
        //         cv::imwrite(else_path, img_row.img);
        //     }
        // }
        ...
        for (const auto& img_row : img_list) {
            ...
            if (img_row.is_detected) {
                // 优先保存检测到的图
                full_path = data_manager_->get_config()->image_dir_ + "/detected" + filename;
            } else if (idx % 5 == 0) {
                // 如果没检测到，但满足抽样频率，保存到 raw
                full_path = data_manager_->get_config()->image_dir_ + "/raw" + filename;
            }

            if (!full_path.empty()) {
                // 建议：这里可以改为把 task 丢进一个 std::thread 或者线程池
...
void DataLogger::close_files() {
    ...
    if (target_file_.is_open()) target_file_.close();
}