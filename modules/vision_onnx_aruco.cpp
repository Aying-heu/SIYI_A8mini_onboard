...
void VisionModule::onnx_aruco(
    ...
    Z_ONNX(frame, board_model_, boxes1, indexes1, conf_cls_names1);
    ...
    for (int idx1 : indexes1) {
        ...
        for (size_t m_idx = 0; m_idx < marker_ids.size(); m_idx++) {
            ...
            for (size_t j = 0; j < draw_corners.size(); j++) {
                ...
                cv::line(frame, draw_corners[j], draw_corners[next], cv::Scalar(0, 0, 0), 10);  // 榛戣壊杈规
...
void VisionModule::onnx_color_corners(
            cv::Mat& frame,
            UAV_Row& uav_state, Gimbal_Row& gimbal_state,
            std::vector<cv::Point3f>& object_points,std::vector<cv::Point2f>& image_points){
    // 2. 运行 ONNX 检测 (板子)
...
void VisionModule::process_loop() {
    ...
    while (running_) {
        ...
        // if(distance_<=3){
        //     onnx_aruco(row.img,uav_state, gimbal_state,object_points,image_points);
        // }
        // if(distance_>=3){
        //     onnx_color_corners(row.img,uav_state, gimbal_state,object_points,image_points);
        // }
        ...
        cv::split(row.img, bgr_channels); // 拆分通道，结果按B、G、R顺序存入vector
        b = bgr_channels[0];
        g = bgr_channels[1];
        r = bgr_channels[2];
        ...
        cv::Mat ycrcb;
        cv::cvtColor(row.img, ycrcb, cv::COLOR_BGR2YCrCb);
        ...
        if (a_center.x != -1) {
            ...
            cv::circle(row.img, a_center, 10, cv::Scalar(0, 0, 255), -1); // 调试用：红点标出 A
            ...
            object_points.push_back(Point3f(-0.25,0.22,0));
            for (int i = 0; i < 4; i++) {
                // 假设 row.img 是 1280x720，原始是 2560x1440
                float out_x = (rect_board[i].x - row.img.cols/2.0f) * 2.0f + 2560/2.0f;
                float out_y = (rect_board[i].y - row.img.rows/2.0f) * 2.0f + 1440/2.0f;
        ...
        cv::polylines(row.img, vector<vector<Point>>{
        ...
        for (size_t m_idx = 0; m_idx < marker_ids.size(); m_idx++) {
            ...
            for (size_t j = 0; j < draw_corners.size(); j++) {
                cv::circle(row.img, draw_corners[j], 10, cv::Scalar(0, 0, 0), -1);  // 榛戣壊瑙掔偣
                int next = (j + 1) % draw_corners.size();
                cv::line(row.img, draw_corners[j], draw_corners[next], cv::Scalar(0, 0, 0), 3);  // 榛戣壊杈规
                cv::Point2f text_pos = draw_corners[j] + cv::Point2f(8, 20);
                cv::putText(
                    row.img,
                    std::to_string(j + 1),       // 瑙掔偣搴忓彿锛?-4锛?                        
