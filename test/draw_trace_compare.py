...
# def read_trajectory_csv(file_path, is_uav=False):
#     """
#     读取轨迹CSV文件
...
#     # 统一列名（适配无人机和目标的不同列名）
#     if is_uav:
#         # 无人机CSV列名：position_x/position_y/position_z
#         df.rename(columns={
#             'position_x': 'x',
#             'position_y': 'y',
#             'position_z': 'z',
...
#     # 转换时间戳为相对时间（秒），方便对比
#     df = df[df['timestamp'] <= 1768628302858484096].copy()
#     df['time_s'] = (df['timestamp'] - 1768376316769504288) * 1e-9
    
#     # 过滤异常值（可选）
#     df = df[(np.abs(df['x']) < 100) & (np.abs(df['y']) < 100) & (np.abs(df['z']) < 100)]
...
#     # 绘制无人机3D轨迹
#     uav_line = ax4.plot(uav_df['x'], uav_df['y'], uav_df['z'], 'r-', label='UAV Trajectory', linewidth=2)
#     # 绘制目标3D轨迹
#     target_line = ax4.plot(target_df['x'], target_df['y'], target_df['z'], 'b-', label='Target Trajectory', linewidth=2)
#     # 标记起点
...
#     # 计算各轴误差
#     error_x = uav_aligned['x'].values - target_interp['x'].values
#     error_y = uav_aligned['y'].values - target_interp['y'].values
#     error_z = uav_aligned['z'].values - target_interp['z'].values
...
#     # === 替换为你的CSV文件路径 ===    17_133504    4436
#     #    1768628295980528288
#     UAV_CSV_PATH = "/home/bsa/A_vision_relate/STAR/data_20260117_134436/uav_states.csv"    # 无人机轨迹CSV
#     TARGET_CSV_PATH = "/home/bsa/A_vision_relate/STAR/data_20260117_134436/new_target/new_target_states_1300000.csv"  # 目标轨迹CSV
    
#     # 读取数据
#     print("正在读取无人机轨迹数据...")
#     uav_df = read_trajectory_csv(UAV_CSV_PATH, is_uav=True)
#     print("正在读取目标轨迹数据...")
#     target_df = read_trajectory_csv(TARGET_CSV_PATH, is_uav=False)
...
if __name__ == "__main__":
    # === 替换为你的CSV文件路径 ===
    UAV_CSV_PATH = "/home/bsa/A_vision_relate/data_20260117_104512/uav_states.csv"    
    # TARGET_CSV_PATH = "/home/bsa/A_vision_relate/data_20260114_153832/new_target/new_target_states_82700000.csv"
    # /home/bsa/A_vision_relate/STAR/data_20260117_133504//new_target/new_target_states_320135808.csv  
    TARGET_CSV_PATH = "/home/bsa/A_vision_relate/data_20260117_104512//new_target/new_target_states_80000000.csv"  
    # TARGET_CSV_PATH = "/home/bsa/A_vision_relate/STAR/data_20260117_133504//target_states.csv"  
