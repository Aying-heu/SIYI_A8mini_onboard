...
## 核心模块说明
- **📷Camera Module**: 
  - 通过 GStreamer 直接将底层 V4L2 视频流硬件解码并转为 BGR 格式，打上纳秒级统一时间戳后推入Data Manager进行缓存。
- **🎯Gimbal Module**: 
  - 通过 UDP 协议与云台通信，以 20Hz 轮询云台当前的 Pitch/Yaw 角度与 Zoom 倍率，打上纳秒级统一时间戳后推入Data Manager进行缓存
  - 从 Data Manager 获取当前目标状态与无人机状态，数据融合，求解追踪角度，下发云台转向指令。
- **🚁UAV Module**: 
  - 基于 ROS 2 节点，订阅 ```/uav1/prometheus/state```，获取无人机自身在世界坐标系下的高精度位置、速度与四元数姿态。
- **👀Vision Module**: 消费者线程：
  - 阻塞等待新图像，获取图像后在 Data Manager 中回溯匹配同一时间戳的云台和无人机状态。通过多种方案获取目标角点信息，单目情况下采用```cv::solvePnP```，双目情况下采用视差法定位，求解相机系下位姿，最后采用严密的矩阵变换：$R_{world}=R_{uav}×R_{gimbal}×R_{cam}$得到目标在世界中的瞬时状态；并将状态发布为 ROS 2 话题```/target1/prometheus/target_detection```。识别方案如下：最终采用方案4：
...
- **📉Target State Predictor**: 定时器驱动。从 Data Manager 中拉取卡尔曼滤波器 与 误差状态机卡尔曼滤波器 ESKF 滤波后的 15Hz 历史数据形成序列（Sequence），输入早期融合的Surf-PatchTST 的 ONNX 预测模型，输出未来 N 帧的三维坐标和姿态轨迹，发布为 ROS 2 ```/target1/prometheus/target_state``` 供规划层使用。
- **📗Data Logger Module**: 
  - 低优先级异步落盘线程。定期将各类缓存中的历史状态（无人机、云台、目标）和检测图像刷入 SSD，用于复盘分析。

## 核心缓冲池 Data Manager 说明
- 图像推入与弹出：
  - 相机来的图像包含时间戳信息，构造```Img_Row```结构体，存储于```raw_img_deque_```中，保存需加锁，保存完释放，同时通知```img_cv_.notify_one()```，通知视觉线程，有新图可以用于检测！
  - 检测线程空闲时间等待新图，当有新图，且```raw_img_deque_```非空时，获取新图，采用后进先出策略，确保检测图像始终为最新；操作需加锁
  - 检测后的图像，被推入```used_img_vector_```，用于Log模块定时落盘。操作需加锁
  - 确保```raw_img_deque_```大小不超过```MAX_IMG_SIZE```；若超过，采用先进先出策略进行弹出；操作需加锁
- 无人机状态的推入、查询与弹出
  - 无人机状态包含时间戳信息，构造```UAV_Row```，存储于红黑树结构的```map```，```uav_history_```中，其键为时间戳，值为对应的```UAV_Row```；操作需加锁
  - 由于存储结构为有序```map```，查询利用时间戳，采用```lower_bound```查询，获取图像时间戳对应的最近的无人机状态
  - 云台的控制需要获取最新的无人机状态，如果```uav_history_```非空，直接返回最后一个值
  - 每次查询后，因为时间是向前走的，所以可以将当次查询的时间戳作为界限，在这之前的数据全部移动到```used_uav_history_```中，后续由log模块取走；且当其大小大于```MAX_UAV_SIZE```时，弹出旧数据给```used_uav_history_```
- 云台信息的推入、查询与弹出
  - 云台状态包含时间戳信息，构造```Gimbal_Row```，存储于红黑树结构的```map```，```gimbal_history_```中，其键为时间戳，值为对应的```Gimbal_Row```；操作需加锁
  - 由于存储结构为有序```map```，查询利用时间戳，采用```lower_bound```查询，获取图像时间戳对应的最近的云台状态
  - 每次查询后，因为时间是向前走的，所以可以将当次查询的时间戳作为界限，在这之前的数据全部移动到```used_gimbal_history_```中，后续由log模块取走；且当其大小大于```MAX_GIMBAL_SIZE```时，弹出旧数据给```used_gimbal_history_```
- 检测线程获取图像、无人机与云台信息
  - 检测线程始终等待```img_cv_```，获取到图像后，在检测之前，从缓冲池获取与图像时间戳相对应的无人机与云台信息；
  - 检测之后，将目标状态给```Data manager```进行滤波器参数更新，并获取更新参数后的卡尔曼滤波状态；发送目标状态，并将目标状态和滤波后的状态推入```Data manager```的```detected_target_msg_```和```filtered_target_msg_```
- 预测线程获取目标数据
  - 预测线程以15Hz的频率从```Data manager```的滤波器中获取滤波状态；并当内部缓存的时间序列长度足够时，以3Hz的频率对时间序列进行预测，预测完毕后并发送，并写入磁盘
- 日志Log线程
  - 以0.3Hz的频率从```Data manager```拿取已经使用过的数据，包括图像，无人机状态，云台状态，目标原始状态和滤波状态，写入磁盘
